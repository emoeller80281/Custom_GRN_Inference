...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--organism", type=str, required=True, help="Name of the emsembl organism, e.g. 'mmusculus' or 'hsapiens'")
    parser.add_argument("--tss_distance_cutoff", type=str, required=False, default="1000000", help="Distance in nucleotide base pairs from the TSS to filter out ATACseq peaks. Default is 1 MB")
...
def load_atac_dataset(atac_data_file: str) -> pd.DataFrame:
    if atac_data_file.lower().endswith('.parquet'):
        df = pd.read_parquet(atac_data_file, nrows=1000)
    ...
    return df.rename(columns={df.columns[0]: "peak_id"}, inplace=True)
...
def main():
    atac_data_file = "/gpfs/Labs/Uzun/SCRIPTS/PROJECTS/2024.SINGLE_CELL_GRN_INFERENCE.MOELLER/input/DS011_mESC/DS011_mESC_sample1/DS011_mESC_ATAC.parquet"
    rna_data_file = "/gpfs/Labs/Uzun/SCRIPTS/PROJECTS/2024.SINGLE_CELL_GRN_INFERENCE.MOELLER/input/DS011_mESC/DS011_mESC_sample1/DS011_mESC_RNA.parquet" 
    organism = "mmusculus"
    ...
    logging.info(f"Loading Ensembl TSS distances for '{organism}'")
    tss_df = load_ensembl_tss_df(organism)
    
    logging.info(f"Filtering out peaks greater than {tss_distance_cutoff / 1e6} MB from the TSS of a gene")
    filtered_atac_df = filter_peaks_within_distance(atac_df, tss_df, tss_distance_cutoff)
    ...
    atac_df_norm = log2_cpm_normalize(filtered_atac_df, label="scATAC-seq", id_col_name="peak_id")
...
if __name__ == "__main__":
    ...
    main()
    # main(args.atac_data_file, args.rna_data_file, args.organism, int(args.tss_distance_cutoff))
