...
def fit_simple_scalers_fast_gpu(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if tf_tensor.dim() == 1:
            tf_values = tf_tensor.reshape(-1)
            tg_values = tg_tensor.reshape(-1)

            if tf_values.numel() != tf_ids.numel() or tg_values.numel() != tg_ids.numel():
                raise ValueError(
                    f"Pairwise batch mismatch: tf_values={tf_values.numel()}, tf_ids={tf_ids.numel()}, "
                    f"tg_values={tg_values.numel()}, tg_ids={tg_ids.numel()}"
                )

            tf_sum.index_add_(0, tf_ids, tf_values)
            tf_sqsum.index_add_(0, tf_ids, tf_values * tf_values)
            tf_count.index_add_(0, tf_ids, torch.ones_like(tf_values, dtype=torch.float32))

            tg_sum.index_add_(0, tg_ids, tg_values)
            tg_sqsum.index_add_(0, tg_ids, tg_values * tg_values)
            tg_count.index_add_(0, tg_ids, torch.ones_like(tg_values, dtype=torch.float32))
        else:
...
class MultiChromosomeDataset(Dataset):
    ...
    @staticmethod
    def collate_fn(batch, max_pairs=None):
        # Reuses the per-chrom collate (expects within-batch consistent W, G, T)
        return MultiomicTransformerDataset.collate_fn(batch, max_pairs=max_pairs)
