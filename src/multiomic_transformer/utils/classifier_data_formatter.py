...
class TrainingDataFormatter:
    ...
    def create_chrom_files(self, total_TG_pseudobulk_global, pseudobulk_chrom_dict, force_recalculate=False, verbose=False):
        ...
        for chrom_id in self.chrom_list:
            ...
            motif_mask_file = self.file_paths["training_cache"][chrom_id]["motif_mask"]
            ...
            motif_mask = self.build_motif_mask(tf_names_kept=tf_names_kept, tg_names_kept=tg_names_kept)
            ...
            torch.save(motif_mask, motif_mask_file)
    ...
    def _load_prior_knowledge_edges(self) -> pd.DataFrame:
        """Load and cache the organism-specific prior-knowledge TF-TG edge list."""
        cached = getattr(self, "_prior_knowledge_edges_df", None)
        if cached is not None:
            return cached

        pkn_file = self.file_paths["genome"]["prior_knowledge_network"]
        edges = pd.read_csv(pkn_file, usecols=["source_id", "target_id"])
        edges["source_id"] = self.canon.canonicalize_series(edges["source_id"].astype(str)).fillna("")
        edges["target_id"] = self.canon.canonicalize_series(edges["target_id"].astype(str)).fillna("")
        edges = edges[(edges["source_id"] != "") & (edges["target_id"] != "")]
        edges = edges.drop_duplicates(subset=["source_id", "target_id"]).reset_index(drop=True)

        self._prior_knowledge_edges_df = edges
        return edges

    def build_motif_mask(self, tf_names_kept: Iterable[str], tg_names_kept: Iterable[str]) -> torch.Tensor:
        """Build a binary [num_tgs x num_tfs] TF-TG interaction mask from the prior-knowledge network."""
        tf_names_kept = [self._canon(tf) for tf in tf_names_kept]
        tg_names_kept = [self._canon(tg) for tg in tg_names_kept]

        motif_mask = torch.zeros((len(tg_names_kept), len(tf_names_kept)), dtype=torch.bool)
        if len(tf_names_kept) == 0 or len(tg_names_kept) == 0:
            return motif_mask

        tf_index = {name: i for i, name in enumerate(tf_names_kept)}
        tg_index = {name: i for i, name in enumerate(tg_names_kept)}

        for source_id, target_id in self._load_prior_knowledge_edges()[["source_id", "target_id"]].itertuples(index=False, name=None):
            tf_idx = tf_index.get(source_id)
            tg_idx = tg_index.get(target_id)
            if tf_idx is not None and tg_idx is not None:
                motif_mask[tg_idx, tf_idx] = True
    ...
    def _setup_file_paths(self):
        ...
        self.file_paths["genome"] = {
            ...
            "prior_knowledge_network": self.project_dir / "data" / "prior_knowledge_network_data" / self.organism_code / f"{self.organism_code}_merged_pkn.csv",
        }
        ...
        for chrom_id in self.chrom_list:
            ...
            motif_mask_file: Path =             chrom_cache_dir / f"motif_mask_{chrom_id}.pt"
    ...
    def check_cached_file_exist(self):
        ...
        for chrom_id in self.chrom_list:
            ...
            chrom_specific_required_files = [
                ...
                chrom_cache_dir / f"motif_mask_{chrom_id}.pt",
    ...
    def _handle_assertions(self):
        ...
        assert pkn_file.is_file(), f"Prior knowledge network file not found: {pkn_file}"
