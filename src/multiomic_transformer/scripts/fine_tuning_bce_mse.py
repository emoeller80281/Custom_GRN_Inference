...
from multiomic_transformer.models.model import MultiomicTransformer, MultiomicTransformerWithHeads
...
def save_tf_tg_embeddings_from_model(model, out_dir, vocab_dir, epoch=None):
    ...
    # If we're wrapped in MultiomicTransformerWithHeads, the backbone is in .base
    ...
    torch.save(
        {
            "tf_emb":       base.tf_identity_emb.weight.detach().cpu(),
            "tg_query_emb": base.tg_query_emb.weight.detach().cpu(),
            "tg_emb":       base.tg_identity_emb.weight.detach().cpu(),
        },
        emb_save_path,
    )
...
class Trainer:
    ...
    def _run_batch(self, batch):
        ...
        targets   = targets.to(device)   # this is UNscaled at this point
        bias      = bias.to(device)
        tf_ids    = tf_ids.to(device)
        tg_ids    = tg_ids.to(device)
        motif_mask = motif_mask.to(device)

        # Keep a copy of raw targets for presence labels
        targets_raw = targets
        ...
        if getattr(self, "tg_scaler", None) is not None:
            targets_scaled = self.tg_scaler.transform(targets, tg_ids)
        else:
        ...
        # ---------- Forward pass with new heads ----------
        with autocast(device_type="cuda", dtype=torch.bfloat16):
            mask_arg = motif_mask if USE_MOTIF_MASK else None
            # MultiomicTransformerWithHeads forward should return:
            # p_logits: [B, G], preds: [B, G], ...
            p_logits, preds, attn, shortcut_contrib, _ = self.model(
                atac_wins,
                tf_tensor,
                tf_ids=tf_ids,
                tg_ids=tg_ids,
        ...
        targets32 = torch.nan_to_num(targets_scaled.float(), nan=0.0, posinf=1e6, neginf=-1e6)

        # =====================================================
        # 1) Presence head: BCE-with-logits on unscaled targets
        # =====================================================
        ...
        # =====================================================
        nonzero_mask = present.bool()  # [B, G]

        if nonzero_mask.any():
            reg_loss = F.mse_loss(preds32[nonzero_mask], targets32[nonzero_mask])
        else:
        ...
        if getattr(self, "tg_scaler", None) is not None:
            with torch.no_grad():
                ...
                if nonzero_mask.any():
                    mse_loss_unscaled = F.mse_loss(
                        preds_u[nonzero_mask], targets_u[nonzero_mask]
                    ).detach()
                else:
                    mse_loss_unscaled = torch.tensor(0.0, device=device)
        else:
            if nonzero_mask.any():
                mse_loss_unscaled = F.mse_loss(
                    preds32[nonzero_mask], targets32[nonzero_mask]
                ).detach()
            else:
                mse_loss_unscaled = reg_loss.detach()

        # =====================================================
        # 3) Correlation penalty on non-zero entries only
        # =====================================================
        if nonzero_mask.any():
            x = preds32[nonzero_mask].view(-1)
            y = targets32[nonzero_mask].view(-1)
            ...
            if sst > eps:
                ...
                r2 = torch.tensor(0.0, device=device, dtype=torch.float32)

            mean_r2 = r2
        else:
            mean_r2 = torch.tensor(0.0, device=device, dtype=torch.float32)
        ...
        corr_penalty = (FINETUNE_CORR_WEIGHT * corr_anneal) * (1.0 - mean_r2)

        # =====================================================
        # 4) Shortcut regularization (unchanged)
        # =====================================================
        device = preds32.device

        shortcut_reg = torch.tensor(0.0, device=device, dtype=torch.float32)
        if getattr(model_for_reg, "use_shortcut", False) and hasattr(model_for_reg, "shortcut_layer"):
            ...
            # reg_val might be a float or a tensor; normalize to a tensor on the right device
            if isinstance(reg_val, torch.Tensor):
                reg_val_t = reg_val.to(device=device, dtype=torch.float32)
            else:
                reg_val_t = torch.tensor(reg_val, device=device, dtype=torch.float32)
            shortcut_reg = reg_val_t * FINETUNE_SHORTCUT_REG_WEIGHT


        # =====================================================
        # 5) EWC penalty (unchanged)
        # =====================================================
        loss_ewc = torch.tensor(0.0, device=device, dtype=torch.float32)
        ...
        # =====================================================
        # 6) Total loss = presence + regression + correlation + regs
        # =====================================================
        total_loss = (
            BCE_WEIGHT * bce_loss
            + REG_WEIGHT * reg_loss
        ...
        # Return BCE so we can track & log it explicitly
        return (
            total_loss,          # full objective
            bce_loss.detach(),   # presence head BCE
            reg_loss.detach(),   # scaled regression MSE (non-zero only)
            mse_loss_unscaled,   # unscaled regression MSE (non-zero only)
            mean_r2.detach(),    # R² of regression on non-zero entries
            corr_penalty.detach(),
            loss_ewc.detach(),
        )
    ...
    def _validate(self):
        ...
        with torch.no_grad():
            for name, loader in self.val_data.items():
                for batch in loader:
                    ...
                    mask_arg = motif_mask if USE_MOTIF_MASK else None
                    with autocast(device_type="cuda", dtype=torch.bfloat16):
                        # New model: returns p_logits, preds, ...
                        p_logits, preds, _, _, _ = self.model(
                            atac_wins,
                            tf_tensor,
                            tf_ids=tf_ids,
                            tg_ids=tg_ids,
        ...
        # If no non-zero entries were seen, bail out
        ...
        return (
            float(avg_loss_scaled),
            float(avg_loss_unscaled),
            float(avg_bce),
            float(r2_s.item()),
            float(r2_u.item()),
            per_dataset_metrics,
    ...
    def _run_epoch(self, epoch):
        ...
        for iteration, (batch, dataset_name) in enumerate(batch_iter):
            ...
            total_bce_sum        += float(bce_loss_val)
        ...
        avg_train_bce = total_bce_sum / max(1, n_batches)
...
def load_train_objs(run_cfg):
    ...
    # --- Step 4: Initialize *base* model (pretrained backbone) ---
    base_model = MultiomicTransformer(
    ...
    # --- Step 5: Load pretrained weights/scalers into the *base* model ---
    ...
    if pretrained_model.exists():
        ...
        if "model_state_dict" in state_dict:
            ...
            # old style checkpoint that was just the state_dict
            base_model.load_state_dict(state_dict, strict=False)

    # --- Step 6: Freeze backbone parameters for transfer learning ---
    for p in base_model.parameters():
        p.requires_grad = False

    # --- Step 7: Wrap with MultiomicTransformerWithHeads ---
    # adjust the constructor to match your class signature
    model = MultiomicTransformerWithHeads(
        base_model=base_model,
    )

    # At this point, only the new heads should have requires_grad=True.
    # Double-check if you want:
    # for name, p in model.named_parameters():
    #     print(name, p.requires_grad)

    # --- Step 8: Fine-tune optimizer (only trainable params = heads) ---
...
def main(rank: int, local_rank: int, world_size: int, save_every: int, total_epochs: int, batch_size: int):
    ...
    try:
        ...
        fine_tune_dir = FINE_TUNING_DIR / "fine_tuning_bce_mse"
