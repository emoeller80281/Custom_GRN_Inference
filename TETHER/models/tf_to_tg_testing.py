...
class TFTGRegulationModel(nn.Module):
    ...
    # peak_attention is called with E*C rows -- batch_size x max_cells_per_pair -- and
    # nn.MultiheadAttention dispatches to scaled_dot_product_attention, whose kernels map
    # that batch dimension onto a CUDA grid dimension capped at 65,535.
    #
    # An ordinary config change crosses that silently. batch_size 1024 with
    # max_cells_per_pair 64 is exactly 65,536, and it dies with "CUDA error: invalid
    # configuration argument" from inside scaled_dot_product_attention -- an error that
    # names neither the batch nor the limit, and which surfaces during Lightning's
    # validation sanity check, before a single training step. Verified on an A100:
    # 65,535 rows OK, 65,536 fails; heads do not enter into it (65,535 x 4 heads is fine).
    #
    # Chunking is exact rather than an approximation: every row attends only to its own
    # keys, so rows are independent and slicing the batch cannot change any output.
    ATTENTION_ROW_CHUNK = 32_768
    ...
    def forward(
        ...
        if EC > self.ATTENTION_ROW_CHUNK:
            chunks = []
            for start in range(0, EC, self.ATTENTION_ROW_CHUNK):
                sel = slice(start, min(start + self.ATTENTION_ROW_CHUNK, EC))
                chunk_context, _ = self.peak_attention(
                    query=tg_query[sel],
                    key=peak_tokens[sel],
                    value=peak_tokens[sel],
                    key_padding_mask=(
                        None if key_padding_mask is None else key_padding_mask[sel]
                    ),
                    need_weights=False,
                )
                chunks.append(chunk_context)
            peak_context = torch.cat(chunks, dim=0)
        else:
