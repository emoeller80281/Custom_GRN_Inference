...
import scripts.train_tf_to_tg_model as tf_tg_trainer
import utils
import config
import warnings
import argparse
...
def make_parser():
    ...
    parser.add_argument("--subsample_number", type=int, required=True, help="Subsample number")
...
def save_multiomic_count_data_to_csv(mdata, rna_csv_file, atac_csv_file, selected_cells):
    rna = mdata.mod["rna"]
    rna_gene_by_cell_counts = pd.DataFrame(rna.layers["counts"].todense().T, index=rna.var_names, columns=rna.obs_names)

    atac = mdata.mod["atac"]
    atac_peak_by_cell_counts = pd.DataFrame(atac.layers["counts"].todense().T, index=atac.var_names, columns=atac.obs_names)

    selected_rna_df = rna_gene_by_cell_counts.loc[:, selected_cells]
    selected_atac_df = atac_peak_by_cell_counts.loc[:, selected_cells]
    
    logging.info(f"RNA cells: {selected_rna_df.shape[1]}, RNA genes: {rna.n_vars}")
    logging.info(f"ATAC cells: {selected_atac_df.shape[1]}, ATAC peaks: {atac.n_vars}")

    logging.info(f"Saving RNA data to {rna_csv_file}")
    selected_rna_df.to_csv(rna_csv_file, index=True)
    
    logging.info(f"Saving ATAC data to {atac_csv_file}")
    selected_atac_df.to_csv(atac_csv_file, index=True)
...
def build_and_save_training_cache(args, paths):
    ...
    try:
        ...
        # ============ CELL SUBSAMPLING FOR stability TESTING ============
        selected_cells_json_file = paths["cache_dir"] / f"selected_cells_subsample_{subsample_number}.json"
        
        if not selected_cells_json_file.exists():
            logging.info(f"Selected cells JSON file does not exist. Generating new subsample and saving to {selected_cells_json_file}")
            mdata = mu.read_h5mu(sample_input_data_dir / "multiome_processed.h5mu")
            ...
            logging.info(f"  - Subsample number: {subsample_number}")
            
            multigrntools_raw_data_dir = Path("/gpfs/Labs/Uzun/DATA/PROJECTS/2024.GRN_BENCHMARKING.MOELLER/MUON_FILTERED_COUNT_DATASETS")
            
            rna_csv_file: Path = multigrntools_raw_data_dir / "stability" / args.cell_type / args.sample_name / f"subsample_{subsample_number}" / f"{args.sample_name}_RNA.csv"
            atac_csv_file: Path = multigrntools_raw_data_dir / "stability" / args.cell_type / args.sample_name / f"subsample_{subsample_number}" / f"{args.sample_name}_ATAC.csv"
            
            rna_csv_file.parent.mkdir(parents=True, exist_ok=True)
            atac_csv_file.parent.mkdir(parents=True, exist_ok=True)
            
            # Save the subsampled RNA and ATAC count data to CSV files for multiGRNtools
            logging.info(f"Saving subsampled RNA and ATAC count data to CSV files for multiGRNtools")
            save_multiomic_count_data_to_csv(mdata, rna_csv_file, atac_csv_file, selected_cells)
            
            # Save a json of the selected cells for reproducibility
            logging.info(f"Saving selected cells for reproducibility to {selected_cells_json_file}")
            utils.atomic_json_dump(selected_cells.tolist(), selected_cells_json_file, indent=2)
        else:
            logging.info(f"Selected cells JSON file already exists. Loading selected cells from {selected_cells_json_file}")
            with open(selected_cells_json_file, "r") as f:
        ...
        rna_pseudobulk_norm = rna_pseudobulk_norm.loc[:, selected_cells]
        atac_pseudobulk = atac_pseudobulk.loc[:, selected_cells]
        
        # Get the number of common cells after subsampling
        ...
        tftg_inputs_train = tf_tg_data_builder.build_tftg_inputs(tf_tg_labeled_train_df, seed=123, silence=False, **common_build_kwargs)
        logging.info("Building validation TF-TG input dataset on rank 0")
        tftg_inputs_val = tf_tg_data_builder.build_tftg_inputs(tf_tg_labeled_val_df, seed=124, silence=False, **common_build_kwargs)
...
def make_dataloader(dataset, *, batch_size, shuffle, num_workers, prefetch_factor):
    kwargs = dict(
        ...
        collate_fn=tf_tg_trainer.collate_tftg_edge_bags,
    )
    if num_workers > 0:
...
def main():
    ...
    train_dataset = tf_tg_trainer.TFTGEdgeBagDataset(
        tftg_inputs_train,
        tf_embeddings_tensor=tf_embeddings_tensor,
        tf_mask_tensor=tf_mask_tensor,
        atac_peak_tensor=atac_peak_tensor,
    )
    val_dataset = tf_tg_trainer.TFTGEdgeBagDataset(
        tftg_inputs_val,
        tf_embeddings_tensor=tf_embeddings_tensor,
        tf_mask_tensor=tf_mask_tensor,
        atac_peak_tensor=atac_peak_tensor,
    )
    test_dataset = tf_tg_trainer.TFTGEdgeBagDataset(
        tftg_inputs_test,
        tf_embeddings_tensor=tf_embeddings_tensor,
        tf_mask_tensor=tf_mask_tensor,
        atac_peak_tensor=atac_peak_tensor,
    ...
    run_name = f"{args.sample_name}_{args.subsample_number}_{args.job_id}"
    paths["output_dir"].mkdir(parents=True, exist_ok=True)
